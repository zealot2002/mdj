...
public class EffectLinearLayout extends LinearLayout {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
			 ...
			 * 取第一个触摸点的位置
			 */
			final int pointerIndexTranslate = event
					.findPointerIndex(fTranslatePointerId);
			if (pointerIndexTranslate >= 0) {
				...
				// Log.i("com.zte.allowance", "fTranslatePointerId = " +
				// fTranslatePointerId);
				/**
				 * deltaX 将要在X轴方向上移动距离 scrollX 滚动deltaX之前，x轴方向上的偏移 scrollRangeX
				 * 在X轴方向上最多能滚动的距离 maxOverScrollX 在x轴方向上，滚动到边界时，还能超出的滚动距离
				 */
				// Log.i("com.zte.allowance", "delta y = " +
				// (fTranslateLastTouch.y - translateY));
				this.overScrollBy((int) (fTranslateLastTouch.x - translateX),
						(int) (fTranslateLastTouch.y - translateY) / 4,
						this.getScrollX(), this.getScrollY(), 0, 0, 0,
						OVERSCROLL_DISTANCE, true);
		...
		case MotionEvent.ACTION_UP: {
			/**
			 * startX 回滚开始时x轴上的偏移 minX 和maxX 当前位置startX在minX和manX之 间时就不再回滚
			 * 
			 * 此配置表示X和Y上的偏移都必须复位到0
			 */
			if (fScroller.springBack(this.getScrollX(), this.getScrollY(), 0,
					0, 0, 0))
				this.invalidate();
	...
	public void computeScroll() {
		if (fScroller != null && fScroller.computeScrollOffset()) {
			...
			/**
			 * 根据动画开始及持续时间计算出当前时间下，view的X.Y方向上的偏移量 参见OverScroller
			 * computeScrollOffset 的SCROLL_MODE
			 */
			...
			if (oldX != x || oldY != y) {
				...
				this.overScrollBy(x - oldX, (y - oldY), oldX, oldY, 0, 0, 0,
						OVERSCROLL_DISTANCE, false);
			}
	...
	protected void onOverScrolled(int scrollX, int scrollY, boolean clampedX,
		...
		if (!fScroller.isFinished()) {
			...
			if (clampedX || clampedY) {
				fScroller.springBack(this.getScrollX(), this.getScrollY(), 0,
						0, 0, 0);
			}
		} else {
			super.scrollTo(scrollX, scrollY);
