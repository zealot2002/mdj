...
public class MySectionIndexer implements SectionIndexer{
    ...
    //是否为当前section的最后一条
    public boolean isLastPositionForSection(int section,int position) {
        if (position < 0 || position >mCount) {
            return false;
        }
        if(position == mCount){//最后一条
            return true;
        }
        if(section+1<mPositions.length){//最后一块的最后一条，在上面已经返回
            return mPositions[section+1]==position+1;
        }
        return false;
    }
//返回第i块的绝对位置
